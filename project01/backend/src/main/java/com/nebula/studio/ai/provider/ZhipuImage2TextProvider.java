...
@Slf4j
@Component
public class ZhipuImage2TextProvider implements Image2TextProvider {
    ...
    @Value("${ai.zhipu.vision-model:GLM-4V-Flash}")
    private String visionModel;

    @Value("${ai.zhipu.api-key:}")
    private String defaultApiKey;

    public ZhipuImage2TextProvider(DynamicConfigService dynamicConfigService) {
    ...
    public Image2TextResult analyze(byte[] imageBytes, String mimeType, String prompt) {
        ...
        if (apiKey == null || apiKey.isBlank()) {
            return Image2TextResult.fail("智谱AI API Key 未配置", getProviderName());
        }
        try {
            ...
            log.error("智谱AI 图生文失败: {}", e.getMessage());
    ...
    public Image2TextResult analyzeUrl(String imageUrl, String prompt) {
        ...
        try {
            ...
            log.error("智谱AI 图生文(URL)失败: {}", e.getMessage());
            return Image2TextResult.fail("分析失败: " + e.getMessage(), getProviderName());
        }
    }
    ...
    private Image2TextResult callVisionApi(String imageData, String prompt, String apiKey) throws IOException {
        ...
        log.info("调用智谱AI图生文, model={}", visionModel);
