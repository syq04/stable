<template>
  <div class="history-page">
    <div class="card">
      ...
      <div class="card-body">
        <el-table :data="records" stripe v-loading="loading" style="width:100%">
          ...
          <el-table-column prop="inputContent" label="提示词" min-width="200" show-overflow-tooltip />
    ...
    <el-dialog v-model="detailVisible" title="生成详情" width="600px">
      <div v-if="currentRecord" class="detail-content">
        <el-image v-if="currentRecord.imageUrl" :src="currentRecord.imageUrl" fit="contain" class="detail-img" />
        <el-descriptions :column="2" border>
          <el-descriptions-item label="提示词" :span="2">{{ currentRecord.inputContent }}</el-descriptions-item>
          <el-descriptions-item label="输出内容" :span="2">{{ currentRecord.outputContent || '-' }}</el-descriptions-item>
...
import { getText2ImageHistory, deleteText2ImageRecord } from '@/api/text2image'
...
function statusLabel(s) {
  return { PENDING: '等待中', PROCESSING: '生成中', SUCCESS: '已完成', COMPLETED: '已完成', FAILED: '失败' }[s] || s
}
