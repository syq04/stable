...
def test_text2image(token):
    ...
    url = BASE_URL + "/api/text2image/generate"
...
def test_image2text(token):
    ...
    url = BASE_URL + "/api/image2text/analyze"
    headers = {"Authorization": "Bearer " + token}
    
    # 图生文需要上传文件，这里只测试接口是否存在
    try:
        ...
        # 预期是400错误（缺少文件），而不是500
        if response.status_code == 400:
            print("图生文接口正常（需要上传文件）!")
            return True
        else:
...
def test_admin_dashboard(token):
    ...
    url = BASE_URL + "/api/system/dashboard/stats"
    headers = {"Authorization": "Bearer " + token}
...
def test_admin_configs(token):
    ...
    url = BASE_URL + "/api/system/configs"
    headers = {"Authorization": "Bearer " + token}
